...
export default function StudentReportPage() {
  ...
  const generateSummary = () => {
    ...
    const highlights: string[] = filtered.slice(0, 5).map((e) => `${new Date(e.date).toLocaleDateString()}: ${e.title}`);
  ...
  const exportJSON = () => {
    ...
    a.href = url;
    a.download = 'internship-report.json';
    a.click();
  ...
  if (!isAuthed) {
    return (
      <div className="max-w-3xl mx-auto p-6">
        <Card className="p-4">
          <p className="text-[#1A1A1A]">Please login as Student to access Report Generator.</p>
        </Card>
      </div>
    );
  }
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="border rounded-md p-3">
                      <div className="text-[#333333]">Total Entries</div>
                      <div className="text-[#1A1A1A] text-xl font-semibold">{summary.totalEntries}</div>
                    </div>
                    <div className="border rounded-md p-3">
                      <div className="text-[#333333]">Total Hours</div>
                      <div className="text-[#1A1A1A] text-xl font-semibold">{summary.totalHours}</div>
                    </div>
                    <div className="border rounded-md p-3">
